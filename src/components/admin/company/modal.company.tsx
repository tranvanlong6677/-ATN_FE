...
const ModalCompany = (props: IProps) => {
  ...
  const [fileImgCompany, setFileImgCompany] = useState<File>();
  ...
  const submitCompany = async (valuesForm: ICompanyForm) => {
    ...
    if (dataInit?._id) {
      ...
      const resUpdate = await callUpdateCompany(
        dataInit._id,
        name,
        address,
        value
      ...
      //create
      const resCreate = await callCreateCompany(name, address, value);
      if (resCreate.data) {
        handleReset();
        reloadTable();
        // upload logo image
